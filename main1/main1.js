...
function renderMainWorkMedia(container, data) {
  ...
  if (ext === "pdf" && window.pdfjsLib) {
    ...
    window.pdfjsLib.getDocument(u).promise.then(async pdf => {
      pdfState.doc   = pdf;
  ...
  async function renderRecommend() {
    ...
    recommendedTools.forEach(tool => {
      ...
      card.addEventListener('click', () => {
        // ✅ url() 헬퍼로 BASE 자동 적용
        window.location.href = url(`/detail_AI/detail_AI.html?tool_ID=${tool.tool_ID}`);
      });
  ...
  function renderWorkCard(category) {
    ...
    if (moreBtn) {
      ...
      moreBtn.onclick = (e) => {
        e.stopPropagation();
        // ✅ url() 헬퍼로 BASE 자동 적용
        window.location.href = url(`/artwork/artwork_post/artwork_post.html?work_id=${encodeURIComponent(data.workId)}`);
      };
    }
    ...
    if (toolEl) {
      ...
      toolEl.querySelector('.btn-more').addEventListener('click', () => {
        // ✅ url() 헬퍼로 BASE 자동 적용
        window.location.href = url(`/detail_AI/detail_AI.html?tool_ID=${data.tool.id}`);
      });
    }
  }
